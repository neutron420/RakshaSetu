...
import { transcribeAudioStream, synthesizeSpeech } from "./aws-voice.service";
...
export async function chatWithRakshaBotAudio(req: Request, res: Response) {
  ...
  try {
    // 1. Transcribe audio to text using AWS Transcribe Streaming
    const message = await transcribeAudioStream(file.buffer);
    console.log(`[Chat API] Transcribed Audio (AWS): "${message}"`);
...
RakshaSetu AI:`,
    ...
    // 4. Convert AI reply to Speech using AWS Polly
    const audioBuffer = await synthesizeSpeech(reply);

    // Return the result
    return ok(res, "Audio chat response generated", { 
...
export async function chatWithRakshaBotTTS(req: Request, res: Response) {
  requireAuth(req);
  ...
  // Use the exact same logic as chatWithRakshaBot to get the reply
  let locationStr = `User Location: Unknown (Remind the user to enable location services if location is important for their request.)`;
  if (latitude && longitude) {
    locationStr = `User Location: Latitude ${latitude}, Longitude ${longitude}\n\nNearby Relief Centers / Hospitals / Shelters:\n`;
    try {
      let centers = await getNearbyCenters(latitude, longitude, 15000);
      if (!centers || centers.length === 0) {
        await fetchAndSaveReliefCenters(latitude, longitude, 15000);
        centers = await getNearbyCenters(latitude, longitude, 15000);
      }
...
RakshaSetu AI:`,
  ...
  try {
    ...
    // Convert AI reply to Speech using AWS Polly
    const audioBuffer = await synthesizeSpeech(reply);

    return ok(res, "TTS chat response generated", { 
      reply,
      audioBase64: audioBuffer.toString("base64")
    });
  } catch (err: any) {
    console.error("[Chat API] TTS Error:", err);
    return res.status(500).json({ success: false, error: "TTS Processing Failed" });
  }
}
